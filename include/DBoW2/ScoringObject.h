...
/// Base class of scoring functions  评分函数基类
...
public:
   ...
   * to the scoring scheme  参数 决定返回的向量返回之前是否必须被归一化
...
/** 
 * Macro for defining Scoring classes  用来计算两个词袋的评分
 * @param NAME name of class
 * @param MUSTNORMALIZE if vectors must be normalized to compute the score
 * @param NORM type of norm to use when MUSTNORMALIZE  反斜杠只是宏定义中的换行 这里快速定义了6个评分类
 */
