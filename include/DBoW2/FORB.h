 ...
 * Description: functions for ORB descriptors   ORB描述子的操作，继承自FClass基类
...
public:
  ...
  /// Descriptor type
  typedef cv::Mat TDescriptor; // CV_8U  一个描述子
  /// Pointer to a single descriptor
  typedef const TDescriptor *pDescriptor;
  /// Descriptor length (in bytes 32个8U类型 256位)
  ...
  /**
   * Returns a mat with the descriptors in float format 将描述子转化为N行(描述子数量)*L(32个8U)的矩阵
   * @param descriptors
