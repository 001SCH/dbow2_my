 ...
 * Description: structure to store results of database queries   用于构造 数据库查询相似性的输出结果  数据结构为：数据库中的条目id-----相似性评分
...
/// Single result of a query 一个查询结果  类
class Result
{
public:
  ...
  /**
   * Creates a result with the given data  根据给定数据创建一个查询结果
   * @param _id entry id    条目Id
   * @param _score score  相似性评分
   */
...
/// Multiple results from a query  一次查询多个结果  类
class QueryResults: public std::vector<Result>
{
public:

  /** 
   * Multiplies all the scores in the vector by factor   将向量中的所有分数乘以因子
   * @param factor
   */
